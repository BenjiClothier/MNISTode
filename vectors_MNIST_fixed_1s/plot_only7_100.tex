...
\nextgroupplot[
 ...
 title={Likelihood Comparison: 1's (in dist) data vs 7's (out dist)},
 ]
...
\addplot[red, thick] table[x=timestep, y=mean, col sep=comma] {vectors_MNIST_fixed_1s/only7_100.csv};
\addlegendentry{OOD 7's}
% Mean ± std shaded region abnormal data
\addplot[name path=upper_cats, draw=none, forget plot]
 table[x=timestep, y=mean_plus_std, col sep=comma] {vectors_MNIST_fixed_1s/only7_100.csv};
\addplot[name path=lower_cats, draw=none, forget plot]
 table[x=timestep, y=mean_minus_std, col sep=comma] {vectors_MNIST_fixed_1s/only7_100.csv};
\addplot[red!20, fill opacity=0.3] fill between[of=upper_cats and lower_cats];
\addlegendentry{OOD 7's std $\pm$}
...
% Difference line
\addplot[green!50!black, thick] table[x=timestep, y=mean_diff, col sep=comma] {vectors_MNIST_fixed_1s/difference_7_100s.csv};
\addlegendentry{Mean difference}
% Optional: add std bands for difference
\addplot[name path=upper_diff, draw=none, forget plot]
 table[x=timestep, y=mean_diff_plus_std, col sep=comma] {vectors_MNIST_fixed_1s/difference_7_100s.csv};
\addplot[name path=lower_diff, draw=none, forget plot]
 table[x=timestep, y=mean_diff_minus_std, col sep=comma] {vectors_MNIST_fixed_1s/difference_7_100s.csv};
...
\end{groupplot}
\end{tikzpicture}
\caption{Top: Comparison of learned (1's) vs OOD (7's) data likelihoods. Bottom: Difference between the two.}
\end{figure}
